...
public interface EventoRepositorio extends JpaRepository<Evento, UUID> {
    @Query("SELECT e FROM evento e LEFT JOIN FETCH e.palestrante a WHERE e.date >= :currentDate")
    ...
    Page<Evento> findFilteredEventos(@Param("title") String title,
                                   @Param("startDate") Date startDate,
                                   @Param("endDate") Date endDate,
                                   Pageable pageable);

    Page<Evento> findFilteredEventos(String title, Date startDate, Date endDate, Pageable pageable);
}
